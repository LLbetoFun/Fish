...
import static com.fun.client.font.FontRenderer.mc;
      ...
      if (entity instanceof LivingEntity && (targetDead || entity.isAlive()) && !entity.equals(mc.player) && (targetInvisible || !entity.isInvisible())) {
         if (targetPlayer && entity instanceof Player entityPlayer) {
            if (!canAttackCheck) {
               ...
               return FunGhostClient.registerManager.vModuleManager.target.isSameTeam(entityPlayer)||
                       !FunGhostClient.registerManager.vModuleManager.target.teams.getValBoolean();
            }
         } else {
